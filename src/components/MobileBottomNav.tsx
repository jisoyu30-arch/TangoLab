    ...
    <>
      ...
      {showMore && (
        <>
          ...
          <div className="md:hidden fixed bottom-16 left-2 right-2 bg-secretary-navy border border-secretary-gold/20 rounded-xl shadow-2xl z-50 p-2">
            {MORE_ITEMS.map(item => (
              <NavLink
                key={item.to}
                to={item.to}
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                    isActive ? 'bg-secretary-gold/10 text-secretary-gold' : 'text-gray-300 hover:bg-white/5'
                  }`
                }
              >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-secretary-navy border-t border-secretary-gold/20 flex flex-row items-center justify-around z-50 px-2 pb-1">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${
                isActive ? 'text-secretary-gold' : 'text-gray-500 hover:text-gray-300'
              }`
            }
          >
        ...
        <button
          ...
          className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${
            isMoreActive || showMore ? 'text-secretary-gold' : 'text-gray-500 hover:text-gray-300'
          }`}
        >
