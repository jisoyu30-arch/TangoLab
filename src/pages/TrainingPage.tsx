          ...
          {tab === 'overview' && (
            <div className="space-y-5">
              ...
              <div className="bg-white/5 border border-tango-brass/10 rounded-xl p-5">
                <h3 className="text-xs font-semibold text-tango-brass mb-3">이번 달</h3>
              ...
              <div className="bg-white/5 border border-tango-brass/10 rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-semibold text-tango-brass">최근 7일 연습</h3>
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <div className="flex-1 w-full flex items-end">
                          <div
                            className={`w-full rounded-t transition-all ${isToday ? 'bg-tango-brass' : 'bg-tango-brass/40'}`}
                            style={{ height: `${height}%` }}
                            title={`${d.minutes}분`}
                          />
                        </div>
                        <div className={`text-[10px] ${isToday ? 'text-tango-brass font-bold' : 'text-gray-500'}`}>
                          {dayLabel}
                        </div>
                  ...
                  .map(item => {
                    ...
                    return (
                      <div key={item.id} className="flex items-center gap-3 px-3 py-2.5 bg-white/5 rounded-lg mb-1">
                        ...
                        {isClass && (
                          <Link to={`/training/class/${item.id}`} className="text-xs text-tango-brass hover:underline">보기</Link>
                        )}
                      </div>
                    );
                  })}
                {classes.length === 0 && practices.length === 0 && (
                  <div className="bg-white/5 border border-dashed border-tango-brass/20 rounded-xl p-6 text-center text-gray-500 text-sm">
  ...
  return (
    <button
      ...
      className={`px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-colors border-b-2 ${
        active ? 'text-tango-brass border-tango-brass' : 'text-gray-500 border-transparent hover:text-gray-300'
      }`}
    >
      {children}
    </button>
...
function Stat({ label, value, unit }: { label: string; value: number; unit: string }) {
  return (
    <div className="text-center">
      <div className="text-2xl font-bold text-tango-brass">{value}</div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {c.topics.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {c.topics.map(t => (
                    <span key={t} className="text-[10px] px-2 py-0.5 bg-tango-brass/10 text-tango-brass rounded-full">{t}</span>
                  ))}
                </div>
              )}
            </div>
    ...
    return (
      <div className="space-y-3">
        <button
          ...
          className="w-full bg-tango-brass/20 hover:bg-tango-brass/30 text-tango-brass rounded-xl p-4 text-center transition-colors min-h-[60px] font-medium"
        >
          + 첫 연습 기록 추가
        </button>
      </div>
        ...
        <div key={p.id} className="bg-white/5 border border-white/10 rounded-xl p-4">
          {editingId === p.id ? (
            <div className="space-y-2">
                ...
                {(['low', 'medium', 'high'] as const).map(lv => (
                  <button key={lv} onClick={() => setEditData({ ...editData, energy_level: lv })}
                    className={`flex-1 px-3 py-1.5 text-xs rounded-lg transition-colors ${
                      editData.energy_level === lv ? 'bg-tango-brass/30 text-tango-brass' : 'bg-white/5 text-gray-400'
                    }`}>
              ...
              <div className="flex gap-2">
                <button onClick={saveEdit} className="flex-1 bg-tango-brass/20 text-tango-brass rounded-lg py-2 text-xs font-medium hover:bg-tango-brass/30">저장</button>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-tango-brass">{p.duration_minutes}분</span>
                    {p.energy_level && (
                  ...
                  {p.focus.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {p.focus.map(f => (
                        <span key={f} className="text-[10px] px-2 py-0.5 bg-tango-brass/10 text-tango-brass rounded-full">{f}</span>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => startEdit(p)} className="text-xs text-gray-500 hover:text-tango-brass px-2 py-1">편집</button>
